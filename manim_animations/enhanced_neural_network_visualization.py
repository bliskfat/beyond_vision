...
class EnhancedNeuralNetwork(Scene):
    def construct(self):
        ...
        hidden_values = self.get_layer_values(hidden_layer[1])
        self.animate_forward_propagation(hidden_layer, output_layer, output_weights)
        output_values = self.get_layer_values(output_layer[1])

        # Show loss calculation
        actual_output = [0, 1]
        self.show_loss_calculation(output_values, actual_output)

        # Animate backpropagation with weight updates
        self.animate_backpropagation(input_layer, hidden_layer, hidden_weights, output_layer, output_weights,
                                     output_values, actual_output)
    ...
    def animate_forward_propagation(self, input_layer, output_layer, weights):
        input_values = self.get_layer_values(input_layer[1])
        new_values = []
        for j, neuron in enumerate(output_layer[0]):
            ...
            equation = MathTex(f"z_{j} = " + " + ".join([f"{input_values[i]:.2f} \\cdot {weights[i][j]:.2f}" for i in
                                                         range(len(input_values))]) + f" = {weighted_sum:.2f}")
            self.play(Write(equation.next_to(neuron, UP)))
            self.play(Transform(output_layer[1][j], Text(f"{activation:.2f}").next_to(neuron, RIGHT)))
            self.wait(0.5)
            self.play(FadeOut(equation))
        return new_values

    def show_loss_calculation(self, output_values, actual_output):
        loss = sum((output_values[i] - actual_output[i]) ** 2 for i in range(len(output_values))) / len(output_values)
        loss_text = Text(f"Loss: {loss:.2f}").move_to(UP)
        self.play(Write(loss_text))
        self.wait(2)
        self.play(FadeOut(loss_text))

    def animate_backpropagation(self, input_layer, hidden_layer, hidden_weights, output_layer, output_weights,
                                output_values, actual_output):
        # This method would include detailed steps of backpropagation (calculating gradients, updating weights).
        # Here we show the concept with a simple indication animation.
        self.play(Indicate(output_layer[0]))
        self.play(Indicate(hidden_layer[0]))
        self.play(Indicate(input_layer[0]))

    def get_layer_values(self, value_labels):
        return [float(label.text) for label in value_labels]
